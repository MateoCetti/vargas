            ...
            </div>
            {
                ...
                <>
                    ...
                    <div className=" flex fixed bg-white rounded-xl w-5/6 h-4/5 inset-y-0 my-auto inset-x-0 mx-auto z-50 flex flex-col items-center p-5">
                        <h1 className="mt-10 text-4xl">Manzanas</h1>

                        <div className="flex jusify-between items-center gap-4 ">
                            <p>manzana equisde</p>
                            <div className="relative flex items-center max-w-[8rem]">
                                <button className="bg-green-700 hover:bg-green-400 border border-gray-300 rounded-s-full p-3 h-11 focus:ring-gray-100 focus:ring-2 focus:outline-none"
                                    onClick={() => setQuantity(quantity - 1)}>
                                    <svg className="w-3 h-3 text-gray-900 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 2">
                                        <path stroke="currentColor" d="M1 1h16" />
                                    </svg>
                                </button>
                                <input type="number" className="bg-gray-50 border-x-0 border-gray-300 h-11 text-center text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 block w-full py-2.5 dark:border-gray-600 dark:placeholder-gray-400  dark:focus:ring-blue-500 dark:focus:border-blue-500" 
                                value={quantity} onChange={(e) => {setQuantity(e.target.valueAsNumber)}} />
                                <button className="bg-green-700  hover:bg-green-400 border border-gray-300 rounded-e-full p-3 h-11 focus:ring-gray-100 focus:ring-2 focus:outline-none"
                                    onClick={() => setQuantity(quantity + 1)}>
